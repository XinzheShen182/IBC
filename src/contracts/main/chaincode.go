...
func (cc *SmartContract) TestOracle(ctx contractapi.TransactionContextInterface, instanceID string, ActivityID string, CID string) (bool, error) {
	// set the CID To ActivityID
	fmt.Print("SetDataItem")
	_, err := cc.Invoke_Other_chaincode(ctx, "test1:v1", "default", oracle.EncodeSetDataItemArgs(
		instanceID, ActivityID, CID, "STRING",
	))
	if err != nil {
		fmt.Print(err)
		return false, err
...
func (cc *SmartContract) TestOracleRead(ctx contractapi.TransactionContextInterface, instanceID string, ActivityID string) (string, error) {
	res, err := cc.Invoke_Other_chaincode(ctx, "test1:v1", "default", oracle.EncodeGetDataItemArgs(
	...
	fmt.Print("dataItem.Value: ")
	fmt.Println(dataItem.Value)
	return string((*dataItem).Value), nil
}

func (cc *SmartContract) PrintHello(ctx contractapi.TransactionContextInterface) (string, error) {
	fmt.Println("Hello")
	return "Hello", nil
}

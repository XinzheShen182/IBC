...
func (cc *SmartContract) InitLedger(ctx contractapi.TransactionContextInterface) error {
	...
	cc.CreateMessage(ctx, "delivered product information", "Participant_1gcdqza", "Participant_0sktaei", "", DISABLED, `{"properties":{"product Id":{"type":"string","description":"Delivered product id"}},"required":["product Id"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "payment amount", "Participant_0sktaei", "Participant_1gcdqza", "", DISABLED, `{"properties":{"payment amount":{"type":"number","description":"payment amount"}},"required":["payment amount"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "External service information", "Participant_0sktaei", "Participant_1gcdqza", "", DISABLED, `{"properties":{"external service Id":{"type":"string","description":"The requested external service information"}},"required":["external service Id"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "Invoice demand", "Participant_1080bkg", "Participant_0sktaei", "", DISABLED, `{"properties":{"invoice":{"type":"boolean","description":"Do you need an invoice?"}},"required":["invoice"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "invoice_file", "Participant_0sktaei", "Participant_1080bkg", "", DISABLED, `{"properties":{"invoice_id":{"type":"string","description":"Invoice Id"},"invoice_data":{"type":"number","description":"Date of invoice issuance"}},"required":["invoice_id"],"files":{"invoice":{"type":"file","description":"Invoice documents"}},"file required":["invoice"]}`)
	cc.CreateMessage(ctx, "invoice information", "Participant_1080bkg", "Participant_0sktaei", "", DISABLED, `{"properties":{"invoice information":{"type":"string","description":"Invoice related information"}},"required":["invoice information"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "delivered product information", "Participant_0sktaei", "Participant_1080bkg", "", DISABLED, `{"properties":{"delivered_product_id":{"type":"string","description":"delivered_product_id"}},"required":["delivered_product_id"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "cancel order information", "Participant_1080bkg", "Participant_0sktaei", "", DISABLED, `{"properties":{"motivation":{"type":"string","description":"Motivation for Canceling orders"}},"required":["motivation"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "payment amount", "Participant_1080bkg", "Participant_0sktaei", "", DISABLED, `{"properties":{"payment amount":{"type":"number","description":"payment amount"},"orderID":{"type":"number","description":"The order id of payment"}},"required":["payment amount","orderID"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "plan confirmation", "Participant_1080bkg", "Participant_0sktaei", "", DISABLED, `{"properties":{"confirmation":{"type":"boolean","description":"Whether to accept the service plan"}},"required":["confirmation"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "Service plan and quotation", "Participant_0sktaei", "Participant_1080bkg", "", DISABLED, `{"properties":{"service plan":{"type":"string","description":"service plan"},"price_quotation":{"type":"number","description":"Price quotation"},"need_external_provider":{"type":"boolean","description":"Whether external service providers are required"}},"required":["service plan","price_quotation","need_external_provider"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "service availability", "Participant_0sktaei", "Participant_1080bkg", "", DISABLED, `{"properties":{"is_available":{"type":"boolean","description":"Is the service available?"}},"required":["is_available"],"files":{},"file required":[]}`)
	cc.CreateMessage(ctx, "The requested service", "Participant_1080bkg", "Participant_0sktaei", "", DISABLED, `{"properties":{"serviceId":{"type":"string","description":"The required service id"}},"required":["serviceId"],"files":{},"file required":[]}`)
...
func (cc *SmartContract) Message_0r9lypd_Send(ctx contractapi.TransactionContextInterface, fireflyTranID string, Is_available bool) error {
	...
	cc.PutState(ctx, "Is_available", Is_available)
...
func (cc *SmartContract) ExclusiveGateway_106je4z(ctx contractapi.TransactionContextInterface) error {
	...
	if Is_available := currentMemory.Is_available; Is_available == true {
		cc.ChangeMsgState(ctx, "Message_1em0ee4", ENABLED)
	}
	if Is_available := currentMemory.Is_available; Is_available == false {
		cc.ChangeGtwState(ctx, "ExclusiveGateway_0hs3ztq", ENABLED)
	}
...
func (cc *SmartContract) Message_1em0ee4_Send(ctx contractapi.TransactionContextInterface, fireflyTranID string, Need_external_provider bool) error {
	...
	cc.PutState(ctx, "Need_external_provider", Need_external_provider)
...
func (cc *SmartContract) Message_0m9p3da_Send(ctx contractapi.TransactionContextInterface, fireflyTranID string, Invoice bool) error {
	...
	cc.PutState(ctx, "Invoice", Invoice)
...
func (cc *SmartContract) ExclusiveGateway_0nzwv7v(ctx contractapi.TransactionContextInterface) error {
	...
	if Invoice := currentMemory.Invoice; Invoice == false {
		cc.ChangeEventState(ctx, "Event_08edp7f", ENABLED)
	}
	if Invoice := currentMemory.Invoice; Invoice == true {
...
func (cc *SmartContract) Gateway_1bhtapl(ctx contractapi.TransactionContextInterface) error {
	...
	if Need_external_provider := currentMemory.Need_external_provider; Need_external_provider == true {
		cc.ChangeMsgState(ctx, "Message_1i8rlqn", ENABLED)
	}
	if Need_external_provider := currentMemory.Need_external_provider; Need_external_provider == false {

...
ParticipantMultiplicityHandler.$inject = ['injector', 'bpmnFactory', 'elementRegistry'];
...
ParticipantMultiplicityHandler.prototype.getActivityShapes = function (participant) {
  return this._elementRegistry.filter(element => is(element, 'bpmn:ChoreographyActivity') && element.bandShapes.some(bandShape => bandShape.businessObject === participant));
};
ParticipantMultiplicityHandler.prototype.toggle = function (context) {
  ...
  if (hasBandMarker(participant)) {
    ...
    let multiplicity = this._bpmnFactory.create('bpmn:ParticipantMultiplicity', {
      maximum: 2
    });

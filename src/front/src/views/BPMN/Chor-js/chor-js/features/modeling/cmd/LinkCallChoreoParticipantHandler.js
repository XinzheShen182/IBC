...
LinkCallChoreoParticipantHandler.$inject = ['moddle'];
...
LinkCallChoreoParticipantHandler.prototype.link = function (element, outerParticipant, innerParticipant) {
  const oldParticipantAssociation = element.participantAssociations.find(pa => pa.outerParticipantRef === outerParticipant);
  if (oldParticipantAssociation) {
    // We remove the old participant association
    element.participantAssociations = element.participantAssociations.filter(pa => pa !== oldParticipantAssociation);
...
LinkCallChoreoParticipantHandler.prototype.preExecute = function (context) {
  ...
  const participantAssociation = context.element.participantAssociations.find(pa => pa.outerParticipantRef === context.outerParticipant);

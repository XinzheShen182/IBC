...
export default class DeleteChoreographyHandler {
  ...
  revert(context) {
    ...
    this._choreoUtil.definitions().rootElements.splice(context.indices.choreoIndex, 0, context.removedChoreo);
    this._choreoUtil.diagrams().splice(context.diagramIndex, 0, context.removedDiagram);
  }
  unlinkCallChoreos(callee) {
    const callers = getAllElementsOfType('bpmn:CallChoreography', this._choreoUtil.choreographies()).filter(caller => caller.calledChoreographyRef === callee);
    callers.forEach(caller => this._modeling.unlinkCallChoreo(caller));
  }
}
DeleteChoreographyHandler.$inject = ['commandStack', 'choreoUtil', 'modeling'];
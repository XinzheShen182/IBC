...
ChoreoLabelEditingProvider.$inject = ['injector', 'canvas'];
ChoreoLabelEditingProvider.prototype.activate = function (element) {
  if (is(element, 'bpmn:Message')) {
    ...
    context.bounds = assign(pick(messageBounds, ['y', 'height']), pick(bandBounds, ['x', 'width']));
    return context;
  } else {
...
ChoreoLabelEditingProvider.prototype.getEditingBBox = function (element) {
  ...
  if (is(element, 'bpmn:ChoreographyActivity')) {
    ...
    if (element.businessObject.loopType !== 'None' || !element.businessObject.loopType || is(element, 'bpmn:CallChoreography') || is(element, 'bpmn:SubChoreography') && element.collapsed) {

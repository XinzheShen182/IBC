...
import { getBandGapIndex, updateBands, getBandKind, updateParticipantBandKinds } from '../../../util/BandUtil';
 ...
 */
export default function CreateParticipantBandHandler(injector, canvas, bpmnFactory, elementFactory, moddle, modeling) {
...
CreateParticipantBandHandler.$inject = ['injector', 'canvas', 'bpmnFactory', 'elementFactory', 'moddle', 'modeling'];
...
CreateParticipantBandHandler.prototype.preExecute = function (context) {
  ...
  if (isUndefined(newBandShape) && !context.delete) {
    ...
    newBandShape = this._elementFactory.createShape({
      ...
      x: 0,
      // dummy values for bounds, these will be set later
      y: 0,
      width: 0,
      height: 0,
      hidden: hidden

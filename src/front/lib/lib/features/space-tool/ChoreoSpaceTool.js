...
ChoreoSpaceTool.$inject = ['injector', 'canvas', 'eventBus'];
...
ChoreoSpaceTool.prototype.getSpaceToolConstraints = function (elements, axis, direction, start, minDimensions) {
  ...
  forEach(resizingShapes, function (resizingShape) {
    ...
    const nonMovingResizingChildren = filter(resizingShape.children, function (child) {
      return !isConnection(child) && !isLabel(child) && !includes(movingShapes, child) && !includes(resizingShapes, child) && !is(child, 'bpmn:Participant') &&
      // We need to skip participants and messages as they will move and scale when the activity moves or scales

...
import { createMessageShape, createMessageFlowSemantics, linkMessageFlowSemantics, unlinkMessageFlowSemantics } from '../../../util/MessageUtil';
...
AddMessageHandler.$inject = ['injector', 'canvas'];
AddMessageHandler.prototype.execute = function (context) {
  ...
  if (isUndefined(messageShape)) {
    messageFlow = createMessageFlowSemantics(this._injector, task, participant);

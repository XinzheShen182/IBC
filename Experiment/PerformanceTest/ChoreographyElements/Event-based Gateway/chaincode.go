...
func (cc *SmartContract) InitLedger(ctx contractapi.TransactionContextInterface) error {
	...
	cc.CreateActionEvent(ctx, "Event_0yn91cf", ENABLE)
	cc.CreateGateway(ctx, "Gateway_1j4s3q0", DISABLE)
	cc.CreateMessage(ctx, "Message_18n7u9x", "Member2.org.comMSP", "Member1.org.comMSP", "", DISABLE, "{\"properties\":{\"1234\":{\"type\":\"string\",\"description\":\"\"}},\"required\":[],\"files\":{},\"file required\":[]}")
	cc.CreateMessage(ctx, "Message_19sc0fg", "Member2.org.comMSP", "Member1.org.comMSP", "", DISABLE, "{\"properties\":{\"4321\":{\"type\":\"string\",\"description\":\"\"}},\"required\":[],\"files\":{},\"file required\":[]}")
	cc.CreateActionEvent(ctx, "Event_06odroi", DISABLE)
	cc.CreateActionEvent(ctx, "Event_0a1pzat", DISABLE)
...
func (cc *SmartContract) Event_0yn91cf(ctx contractapi.TransactionContextInterface) error { 
	stub := ctx.GetStub()
	actionEvent, err := cc.ReadEvent(ctx, "Event_0yn91cf")
	if err != nil {
		return err
	}
	...
	cc.ChangeEventState(ctx, "Event_0yn91cf", DONE)
	stub.SetEvent("Event_0yn91cf", []byte("Contract has been started successfully"))

	cc.ChangeGtwState(ctx, "Gateway_1j4s3q0", ENABLE)
	return nil
}

func (cc *SmartContract) Gateway_1j4s3q0(ctx contractapi.TransactionContextInterface) error { 
	stub := ctx.GetStub()
	gtw, err := cc.ReadGtw(ctx, "Gateway_1j4s3q0")
	if err != nil {
		return err
	}
	...
	cc.ChangeGtwState(ctx, "Gateway_1j4s3q0", ENABLE)
	stub.SetEvent("Gateway_1j4s3q0", []byte("EventbasedGateway has been done"))

        cc.ChangeMsgState(ctx, "Message_18n7u9x" ,DISABLE)

        cc.ChangeMsgState(ctx, "Message_19sc0fg" ,DISABLE)


return nil
}

func (cc *SmartContract) Message_18n7u9x_Send(ctx contractapi.TransactionContextInterface, fireflyTranID string) error {
	stub := ctx.GetStub()
	msg, err := cc.ReadMsg(ctx, "Message_18n7u9x")
	if err != nil {
		return err
	}
	...
	stub.PutState("Message_18n7u9x", msgJSON)
		stub.SetEvent("ChoreographyTask_1kvqvlh", []byte("Message wait for confirming"))
...
func (cc *SmartContract) Message_18n7u9x_Complete(ctx contractapi.TransactionContextInterface) error {
	stub := ctx.GetStub()
	msg, err := cc.ReadMsg(ctx, "Message_18n7u9x")
	if err != nil {
		return err
	...
	cc.ChangeMsgState(ctx, "Message_18n7u9x", DONE)
	stub.SetEvent("Message_18n7u9x", []byte("Message has been done"))

	cc.ChangeEventState(ctx, "Event_06odroi" ,ENABLE)


return nil
}	//编排任务的最后一个消息

func (cc *SmartContract) Message_19sc0fg_Send(ctx contractapi.TransactionContextInterface, fireflyTranID string) error {
	stub := ctx.GetStub()
	msg, err := cc.ReadMsg(ctx, "Message_19sc0fg")
	if err != nil {
		return err
	}
	...
	msg.FireflyTranID = fireflyTranID
	msgJSON, _ := json.Marshal(msg)
	stub.PutState("Message_19sc0fg", msgJSON)
		stub.SetEvent("ChoreographyTask_1lmd4t4", []byte("Message wait for confirming"))
...
func (cc *SmartContract) Message_19sc0fg_Complete(ctx contractapi.TransactionContextInterface) error {
	stub := ctx.GetStub()
	msg, err := cc.ReadMsg(ctx, "Message_19sc0fg")
	if err != nil {
		return err
	...
	cc.ChangeMsgState(ctx, "Message_19sc0fg", DONE)
	stub.SetEvent("Message_19sc0fg", []byte("Message has been done"))

	cc.ChangeEventState(ctx, "Event_0a1pzat" ,ENABLE)


return nil
}	//编排任务的最后一个消息

func (cc *SmartContract) Event_06odroi(ctx contractapi.TransactionContextInterface) error { 
	stub := ctx.GetStub()
	event, err := cc.ReadEvent(ctx, "Event_06odroi")
	if err != nil {
		return err
	}
	...
	cc.ChangeEventState(ctx, "Event_06odroi", DONE)
	stub.SetEvent("Event_06odroi", []byte("EndEvent has been done"))
	return nil
}

func (cc *SmartContract) Event_0a1pzat(ctx contractapi.TransactionContextInterface) error { 
	stub := ctx.GetStub()
	event, err := cc.ReadEvent(ctx, "Event_0a1pzat")
	if err != nil {
		return err
	}
	...
	cc.ChangeEventState(ctx, "Event_0a1pzat", DONE)
	stub.SetEvent("Event_0a1pzat", []byte("EndEvent has been done"))
	return nil
}
